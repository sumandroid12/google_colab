...
def save_model(model,dirname,epoch):
    ...
    torch.save( model.state_dict() , '{}/{}_epoch{}.pth'.format(dirname,type(model).__name__,1 ) )
...
def save_optimizer(optimizer,model,dirname,epoch):
    ...
    torch.save( optimizer.state_dict() , '{}/{}_epoch{}.pth'.format(dirname,type(optimizer).__name__ +'_' +type(model).__name__,1 ) )
